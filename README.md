...
Following are the instructions to setup a dev environment for soqonicatalogue.
...
git clone https://github.com/abdvswmdr/soqonicatalogue.git
...
#echo "172.20.0.2 soqonicatalogue-db" >> /etc/hosts
...
### Deploy static  assets
  ...
  * Fetch the latest [release artifact from this URL](https://github.com/abdvswmdr/soqonicatalogue/releases)
e.g.
```
wget -c https://github.com/abdvswmdr/soqonicatalogue/archive/1.0.1.tar.gz
...
cd soqonicatalogue-1.0.1
cp -r  images  /images
```


### Deploy the Soqonicatalogue App

Create releases directory

```
mkdir /opt/apps/soqonicatalogue/releases
...
```
cd  /opt/apps/soqonicatalogue/releases

wget -c https://s3.amazonaws.com/sod-openedx/catalogue/catalogue
```


where **soqonicatalogue** is a binary

create a symlink
```
ln -s /opt/apps/soqonicatalogue/releases/soqonicatalogue /opt/

```

Launch the app
```
/opt/soqonicatalogue -port 7081 -images=/images/ -DSN 'dbuser:password@tcp(localhost:3306)/databasename'
```

e.g.
```
/opt/soqonicatalogue -port 7081 -images=/images/ -DSN 'devops:GKkdw72Jil0ld@tcp(localhost:3306)/soqonicatalogue'
...
The result is a binary named `soqonicatalogue`, in the current directory.
...
If you followed to Go build instructions, you should have a "soqonicatalogue" binary in $GOPATH/src/github.com/microservices-demo/soqonicatalogue/cmd/cataloguesvc/.
To run it use:
```
./soqonicatalogue
```
...
`curl http://localhost:8080/soqonicatalogue`
